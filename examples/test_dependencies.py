...
from xtdamo.dependencies import DependencyChecker, check_dependency, get_available_dependencies, get_missing_dependencies
...
class TestDependencyChecker:
    ...
    def test_predefined_constants(self):
        """测试预定义常量"""
        from xtdamo.dependencies import CRYPTO_AVAILABLE, WIN32_AVAILABLE, WIN32GUI_AVAILABLE
        ...
        # 验证预定义常量与检查结果一致
        assert check_dependency('cryptography') == CRYPTO_AVAILABLE
        assert (check_dependency('win32cred') and check_dependency('win32con')) == WIN32_AVAILABLE
        assert check_dependency('win32gui') == WIN32GUI_AVAILABLE


if __name__ == '__main__':
    test = TestDependencyChecker()
    test.test_check_dependency()
    test.test_check_dependencies()
    test.test_get_available_dependencies()
    test.test_get_missing_dependencies()
    test.test_get_dependency_info()
    test.test_get_installation_commands()
    test.test_dependency_consistency()
